 ...
 * announces itself as `application/xml`, but the payload stays JSON to
 * preserve type fidelity (numbers, booleans, nested arrays) without writing
 * a bespoke XML schema for every node/edge variant.
 *
...
/**
 * Extracts the JSON payload from an `.easydraw` XML envelope.
 * Returns null if the input isn't a recognizable envelope so callers can
