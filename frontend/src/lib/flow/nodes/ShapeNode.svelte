...
>
	...
	{:else if variant.kind === 'svg'}
		<svg class="shape-fill" preserveAspectRatio="none" viewBox="0 0 100 100" aria-hidden="true">
			{#if shapeType === 'EllipseNode' || shapeType === 'CircleNode'}
				<ellipse
					cx="50"
					cy="50"
					rx="49.5"
			...
			{:else if shapeType === 'DiamondNode'}
				<polygon
					...
					vector-effect="non-scaling-stroke"
				/>
			{:else if shapeType === 'ParallelogramNode'}
				<polygon
			...
			{:else if shapeType === 'OctagonNode'}
				<polygon
					...
					vector-effect="non-scaling-stroke"
				/>
			{:else if shapeType === 'OrthogonalTriangleNode'}
				<!-- Right angle at the bottom-left; hypotenuse runs top-left →
				     bottom-right. -->
				<polygon
					points="1,1 99,99 1,99"
					fill={fillColor}
					stroke={strokeColor}
					stroke-width={strokeWidth}
					stroke-linejoin="round"
					vector-effect="non-scaling-stroke"
				/>
			{:else if shapeType === 'PentagonNode'}
				<polygon
					points="50,1 99,38 80,99 20,99 1,38"
					fill={fillColor}
					stroke={strokeColor}
					stroke-width={strokeWidth}
					stroke-linejoin="round"
					vector-effect="non-scaling-stroke"
				/>
			{:else if shapeType === 'HalfCircleNode'}
				<!-- Bottom half: flat top on the bbox edge, elliptical arc down to y=99. -->
				<path
					d="M1,1 A49,98 0 0 0 99,1 Z"
					fill={fillColor}
					stroke={strokeColor}
					stroke-width={strokeWidth}
					stroke-linejoin="round"
			...
			{:else if shapeType === 'StarNode'}
				<polygon
					...
					vector-effect="non-scaling-stroke"
				/>
			{:else if shapeType === 'ArrowRightNode'}
				<!-- Block arrow →: shaft between y=30..70, head from x=60 to the tip. -->
				<polygon
					points="1,30 60,30 60,1 99,50 60,99 60,70 1,70"
					fill={fillColor}
					stroke={strokeColor}
					stroke-width={strokeWidth}
					stroke-linejoin="round"
					vector-effect="non-scaling-stroke"
				/>
			{:else if shapeType === 'TriangleNode'}
				<!--
				...
				<polygon
					...
					vector-effect="non-scaling-stroke"
				/>
			{:else if shapeType === 'DocumentNode'}
				<!--
			...
			{:else if shapeType === 'ArrowRightNode'} 
				<polygon
					...
					vector-effect="non-scaling-stroke"
				/>
			{:else if shapeType === 'ArrowLeftNode'} 
				<polygon
					points="99,30 40,30 40,1 1,50 40,99 40,70 99,70"
					fill={fillColor}
					stroke={strokeColor}
					stroke-width={strokeWidth}
					stroke-linejoin="round"
					vector-effect="non-scaling-stroke"
				/>
			{:else if shapeType === 'ArrowUpNode'}
				<polygon
					points="30,99 30,40 1,40 50,1 99,40 70,40 70,99"
					fill={fillColor}
					stroke={strokeColor}
					stroke-width={strokeWidth}
					stroke-linejoin="round"
					vector-effect="non-scaling-stroke"
				/>
			{:else if shapeType === 'DatabaseNode'}
				<!--
					Database cylinder. The silhouette fill is one closed path:
					vertical sides at x=1/99, a top cap (back rim, arcs UP through
					(50,1)) and a front-bulging base (arcs DOWN through (50,99)).
					cap/base half-height ry=12. A second stroke-only arc draws the
					cap's FRONT rim (bulging down through the cap) so the top reads
					as a 3D ellipse lip. Non-scaling stroke keeps the outline even
					while preserveAspectRatio="none" stretches the ellipses to fit.
				-->
				<path
